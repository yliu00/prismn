...
class QwenShardingAdapter(ShardingAdapter):
    ...
    Includes QK-norm parameters in attention and AWQ quantization support.
    """
    ...
    def __init__(self, config: PretrainedConfig) -> None:
        ...
        if self.is_awq:
            print(
                "🔧 AWQ quantization detected in Qwen model - using AWQ-specific fusion (dim=1)"
            )
    ...
    def _cat_cols(*tensors: torch.Tensor) -> torch.Tensor:
        # For AWQ packed tensors: shapes [K, N/pack], fuse along dim=1 (cols).
    ...
    def shard_layer(
        ...
        if self.is_awq:
            ...
            # Float weights path (original code)
            # Attention: fuse qkv
        ...
        # QK normalization (unique to Qwen3, same for both AWQ and float)
    ...
    def shard_lm_head(
        ...
        if self.is_awq:
            ...
            if "lm_head.weight" in hf_weights:
                ...
                print(
                    f"✅ Found unquantized lm_head.weight in AWQ Qwen model (size: {hf_weights['lm_head.weight'].shape})"
                )
        else:
